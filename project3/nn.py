...
#     # print initialDataMatrix[0], labelVector[0]
...
def preprocess():
    ...
    _data = fetchdata.get_data('project3_dataset1.txt')

    len_train = int(math.floor(len(_data) * 0.7))
    len_val = int(math.floor(len_train + len(_data) * 0.15))
    # print len_train, len_val

    #Train
    for i in range(len_train):
        # trainx = mat.get('train'+str(i))
        # trainx = np.array(_data[i])
        res = []
        for item in _data[i]:
            try:
                x = float(item)
                ...
                if item == "Absent":
                    ...
                    res.append(1)
        trainx = np.array(res)
        train_data = np.vstack((train_data, trainx[:-1])) if train_data.size else trainx[:-1]
        train_label = np.append(train_label, trainx[-1])
        ...
        for item in _data[i]:
            try:
                ...
                if item == "Absent":
                    ...
                    res.append(1)
        valx = np.array(res)
        validation_data = np.vstack((validation_data,valx[:-1])) if validation_data.size else valx[:-1]
        validation_label =  np.append(validation_label, valx[-1])
    # print validation_data.size, validation_label.size

    #Test data -
    for i in range(len_val, len(_data)):
        res = []
        for item in _data[i]:
            try:
                x = float(item)
                ...
                if item == "Absent":
                    ...
                    res.append(1)
        testx = np.array(res)
        test_data = np.vstack((test_data,testx[:-1])) if test_data.size else testx[:-1]
        test_label =  np.append(test_label, testx[-1])
...
def nnObjFunction(params, *args):
    ...
    # print training_data.shape, w1.T.shape, w2.shape
