...
# generate the susets of arr
def genSub(arr, l):
	subs = []
	# use combinations library from iteretools
	subs.extend(combinations(arr, l))
	subs = map(frozenset, subs)
	return subs
...
# fetching data from with file name
	...
	with open(fname) as f:
		for line in f:
			...
			for i in range(1, len(words) - 1): 
				...
				itemset.add(gene_structure)
			# add disease name
			diseasename = words[-1].replace('\n','')
			sample.add(diseasename)
			sampleDB.append(sample)
			itemset.add(diseasename)
			# add the transactions
			transaction.append(sample)
			# print words[-1] # this is the disease name
	return sampleDB
		
# initializes the code
def init(min_support):
	#read the document
	sampleDB = getDataFromFile('association-rule-test-data.txt')
	# for each entry in geneDB update the count if it exists
	itemmap = {}
	for sample in sampleDB:
		for gene in sample:
			if gene in itemmap:
				...
				# new entry into the itemmap
				itemmap[gene] = 1
	items = itemmap.iteritems()
	# list = filter(lambda k : itemmap[k] < min_support, itemmap)
	list = filter(lambda k : itemmap[k] < min_support, itemmap)
	deleteListFromItemset(list, itemmap)

# delete list from Itemset
def deleteListFromItemset(list, itemmap):
	# items to be removed are in the list are deleted and removed from itemset
	for k in list:
		del itemmap[k]
		itemset.remove(k)
	# print itemset
...
def calculateApriori(gen, min_support, min_conf):
    ...
    l = []
    for key, value in allItems.items()[1:]: # since the k is starting fron 2
        for item in value:
            ...
            for element in tempset:
                ...
                if len(remain) > 0:
                    ...
                    if confidence >= min_conf:
                        l.append(((list(element), list(remain)),
                                           (support, confidence)))
    return l
...
def template3(t, noun1, verb1, geneList1, noun2, verb2, l2, rule_with_val):
    ...
    if t == '1or1':
        ...
        print result1
        # print set(result1)
        print "count of values is " + str(cnt)
...
if __name__ == "__main__":
    ...
    # template3("1or1", "RULE", "ANY", ['Gene1_UP'], "HEAD", 1, ['Gene59_UP'], rule_with_val)
...
def main():
	# If the number of transactions is 5, your minimum support count would be 5*60/100 = 3.
	min_support = 30
	init(min_support)
	# print len(itemset)
	# print 'gene72_down' in itemset

	# candidate count
	candidateCount = 2 

	# do until there is not item in itemset
	while len(itemset) != 0:
		itemmap = {}
		removeset = set()
		# generate new keys for itemmap with combinations
		itemmap_keys = genSub(itemset, candidateCount)
		for k in itemmap_keys:
			countKeys = 0
			for t in transaction:
				if k.issubset(t):
					countKeys += 1
			# add the k into the itemmap
			itemmap[k] = countKeys
			# else:
			# 	# print k
			# 	removeset.add(k)
		_itemset = []
		for item, val in itemmap.items():
			if val >= min_support:
				l = list(item)
				_itemset.append(l)
		_itemlist = list(chain(*_itemset))
		# print _itemlist
		removelist = []
		for item in itemset:
			if item not in _itemlist:
				removelist.append(item)
		print len(removelist)
		map(lambda k: itemset.remove(k), removelist)
		print len(itemset)
		candidateCount += 1
		print "candidate count : {0}".format(candidateCount)
...
def test():
	# print map(set, [x for x in genSub([1, 2, 3, 4], 2)])
	subs = map(set, genSub([1, 2, 3, 4], 3))
	print subs
	print subs[0].issubset(set([7, 8, 9, 1, 2, 3]))
